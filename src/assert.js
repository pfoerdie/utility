...
assert.boolean = function (value) {
  if (!util.is.boolean(value)) throwErr(assert.number, TypeError, 'expected to be a boolean')
  return assert
}
...
assert.number = function (value, min = -Infinity, max = Infinity) {
  if (!util.is.number(value)) throwErr(assert.number, TypeError, 'expected to be a number')
...
assert.number.integer = function (value, min = Number.MIN_SAFE_INTEGER, max = Number.MAX_SAFE_INTEGER) {
  if (!util.is.number.integer(value)) throwErr(assert.number.integer, TypeError, 'expected to be an integer')
...
assert.string = function (value, pattern, min = 0, max = Number.MAX_SAFE_INTEGER) {
  if (!util.is.string(value)) throwErr(assert.string, TypeError, 'expected to be a string')
...
assert.string.iri = function (value) {
  if (!util.is.string(value)) throwErr(assert.string.iri, TypeError, 'expected to be a string')
  if (!util.pattern.iri.test(value)) throwErr(assert.string.iri, Error, 'expected to match iri pattern')
  return assert
}
...
assert.function = function (value) {
  if (!util.is.function(value)) throwErr(assert.function, TypeError, 'expected to be a function')
  return assert
}
...
assert.object = function (value, checkObj) {
  if (!util.is.object(value)) throwErr(assert.object, TypeError, 'expected to be an object')
  if (checkObj) {
...
assert.array = function (value, checkFn, min = 0, max = Number.MAX_SAFE_INTEGER) {
  if (!util.is.array(value)) throwErr(assert.array, TypeError, 'expected to be an array')
  if (checkFn) {
    for (let i = 0; i < value.length; i++) { if (!checkFn(value[i], i)) throwErr(assert.array, Error, 'expected entry [' + i + '] is invalid') }
  }
